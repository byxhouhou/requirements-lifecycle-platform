...
- Beyond Compare 入口可自动检测本机安装，也支持输入或选择程序路径及左右文档路径。
...
2. 可以使用本机文件选择器重新选择 Beyond Compare 程序。
3. 分别输入或选择左侧、右侧文档的完整路径。
4. 点击“打开 Beyond Compare 对比”。

该功能仅由 `ReqFlow.exe` 的回环本地接口执行。文档路径不会上传，也不会由浏览器直接启动任意程序；启动器会验证程序和两个文档均真实存在。
