...
- 支持选择单个文件、多个文件或整个文件夹。
- 支持创建“一项目一仓库”的本地 Git 需求项目，不配置任何远程连接。
- 每次正式基线自动生成 Git commit、注释 tag 和结构化元数据。
...
本地 Git 模式需要工作电脑安装 Git for Windows。SYE 不使用 GitHub，也不会自动配置远程仓库。项目默认保存在：

```text
%LOCALAPPDATA%\ReqFlow\projects\项目名称\
├── .git\
├── project.json
├── requirements\          # 当前版本原始需求文档
└── metadata\baselines\    # 每次基线的结构化元数据
```

在需求输入页填写项目名称并点击“创建 Git 项目”。提交基线时，SYE 将当前文档复制到仓库、执行 `git add` 和 `git commit`，随后创建 `baseline-版本号` 标签。同名项目会打开已有仓库，不会重新初始化。
